...
export function ChabadFinancePage() {
  const rows = useLiveQuery(
    () => db.chabadFinance.orderBy('date').reverse().toArray(),
    [],
  )
  ...
  useEffect(() => {
    void (async () => {
      const inc = await getSetting('chabadIncomeCategories')
      const exp = await getSetting('chabadExpenseCategories')
      setIncomeCats(parseCategories(inc, ['תרומות', 'מכירות', 'אחר']))
      setExpenseCats(parseCategories(exp, ['אירועים', 'ציוד', 'מזון', 'תחבורה', 'אחר']))
    })()
  }, [])
  ...
  async function remove(id?: number) {
    ...
    await db.chabadFinance.delete(id)
  }
  ...
  return (
    <div className="grid" style={{ gap: '1.25rem' }}>
      ...
      <section className="panel">
        <h2>רשומה חדשה — כספי בית חב״ד</h2>
        <p className="muted">נפרד מכספי הבית הפרטי.</p>
            ...
            <tbody>
              {(rows ?? []).map((r) => (
                <tr key={r.id}>
                  ...
                  <td>{r.category}</td>
                  <td style={{ color: r.type === 'income' ? 'var(--ok)' : 'var(--danger)' }}>
